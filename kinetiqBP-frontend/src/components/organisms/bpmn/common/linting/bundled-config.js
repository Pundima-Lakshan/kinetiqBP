...
var conditionalFlows = function () {
  function check(node, reporter) {
    ...
    outgoing.forEach((flow) => {
      const missingCondition = !hasCondition$3(flow) && !isDefaultFlow$2(node, flow);
...
var labelRequired = function () {
  function check(node, reporter) {
    ...
    if (is$d(node, 'bpmn:SequenceFlow') && !hasCondition$2(node)) {
      return;
    }
...
var noDisconnected = function () {
  function check(node, reporter) {
    if (!isAny$2(node, ['bpmn:Task', 'bpmn:Gateway', 'bpmn:SubProcess', 'bpmn:Event']) || node.triggeredByEvent) {
      return;
    }
...
const implementationAttributes = ['camunda:expression', 'camunda:delegateExpression', 'camunda:class', 'camunda:type'];
